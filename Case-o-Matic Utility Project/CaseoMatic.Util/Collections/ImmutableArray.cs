...
namespace Caseomatic.Util
{
    ...
    public struct ImmutableArray<T> : IEquatable<ImmutableArray<T>>, IList
    {
        /// <summary>
        ...
        /// <summary>
        /// Checks if the given object is equal to this immutable array.
        /// </summary>
        /// <param name="obj">The object of type ImmutableArray <typeparamref name="T"/></param>
        /// <returns>A bool that indicates if the parameter is equal.</returns>
        public override bool Equals(object obj)
        {
            var self = this;
            self.ThrowInvalidOperationIfNotInitialized();

            var imArray = (ImmutableArray<T>)obj;
            return self == imArray;
        }
        ...
        public ImmutableArray<T> Insert(int index, T item)
        {
            ...
            if (self.Length == 0)
            {
                return new ImmutableArray<T>(new T[] { item });
            }
        ...
        {
            var self = this;
            int index = self.IndexOf(oldValue, 0, Length, equalityComparer);
            if (index < 0)
            {
        ...
        public ImmutableArray<T> Remove(T item, IEqualityComparer<T> equalityComparer)
        {
            var self = this;
            self.ThrowNullRefIfNotInitialized();
            int index = self.IndexOf(item, 0, Length, equalityComparer);
            return index < 0
        ...
        public ImmutableArray<T> RemoveRange(IEnumerable<T> items)
        {
            return this.RemoveRange(
                new ImmutableArray<T>(items.ToArray()), EqualityComparer<T>.Default);
        }
        ...
        IEnumerator IEnumerable.GetEnumerator()
        {
            var self = this;
            self.ThrowInvalidOperationIfNotInitialized();
            return self.array.GetEnumerator();
        }
        ...
        private void ThrowInvalidOperationIfNotInitialized()
        {
            if (this.IsDefault)
            {
                throw new InvalidOperationException("The immutable array is null.");
            }
        }
